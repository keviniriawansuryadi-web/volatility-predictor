...
print(f"\n{'='*65}")
print("  VIX TERM STRUCTURE FEATURES — SPY")
print(f"{'='*65}")
...
# ── Results ───────────────────────────────────────────────────────────────────
print(f"\n{'='*65}")
print("  FEATURE IMPORTANCE — vix_level changes when term structure added")
print(f"{'='*65}")
vix_level_base = (
    float(fi_base.loc[fi_base["feature"] == "vix_level", "importance"].values[0])
    if "vix_level" in fi_base["feature"].values else float("nan")
)
vix_level_ts = (
    float(fi_ts.loc[fi_ts["feature"] == "vix_level", "importance"].values[0])
    if "vix_level" in fi_ts["feature"].values else float("nan")
)
