...
def print_dashboard(dashboard_data: dict[str, Any]) -> None:
    ...
    for ticker, det in sorted(tickers.items(), key=lambda x: -x[1]["ensemble_forecast"]):
        ...
        print(f"  {ticker:<8} {regime:<10} {ensemble:>6.1%} {rv21d:>6.1%} "
              f"{dis_flag}{dis_val:>4.2f} {vrp_state:<22}  {run_str}")
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*65}")
    print("  SECTION 12 — MARKET DASHBOARD")
    print(f"{'='*65}")
    ...
    if not all_results:
        ...
        print("  Run the main pipeline first (e.g. python main.py --ticker SPY).")
        sys.exit(1)
