...
def run_ticker(
    ...
    def _regime(v: float) -> str:
        ...
        if v > 0.25:
            return "HIGH     -- Strong scalping conditions"
        if v > 0.15:
            return "ELEVATED -- Decent intraday movement expected"
        if v > 0.10:
            return "MODERATE -- Selective scalps only"
...
def main():
    ...
    if args.all_tickers:
        ...
        print("  Metrics saved to outputs/results/{{ticker}}/metrics.csv")
